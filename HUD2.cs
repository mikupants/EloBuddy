...
namespace BigFatHUD
{
    ...
    public class HUD2
    {
        ...
        public HUD2(AIHeroClient hero, int number = 0)
        {
            ...
            else loadondeath = false;
            lastdeath = 0f;

            var xadd = number * 110f * Menus.HUDScale;
            var yadd = number * 110f * Menus.HUDScale;

            var scal = Menus.HUDScale;

            var bmp = Resources.ResourceManager.GetObject(hero.ChampionName + "_Square_0") as Bitmap;
            var image = new Render.Sprite(bmp, new Vector2(0, yadd));

            var bmp2 = Resources.main as Bitmap;
            var image2 = new Render.Sprite(bmp2, new Vector2(0, yadd));

            var RON = Resources.Ron as Bitmap;
            Ronimage = new Render.Sprite(RON, new Vector2(0, yadd));

            var ROFF = Resources.Roff as Bitmap;
            var Roffimage = new Render.Sprite(ROFF, new Vector2(0, yadd));

            HPline = new Render.Line(new Vector2(0, yadd), new Vector2(0, yadd), (int)(8f * Menus.HUDScale), Menus.HighHPBarColor)
            {
                VisibleCondition =
                    sender =>
                        Menus.HUDEnable,
                ...
                {
                    Vector2 v2 = new Vector2(Menus.HUDX + (7 * scal), Menus.HUDY + yadd + (64.5f * scal));
                    return v2;
                },
            ...
            var damageline = new Render.Line(new Vector2(0, yadd), new Vector2(0, yadd), (int)(8f * Menus.HUDScale), Menus.DamgeBarColor)
            {
                VisibleCondition =
                    sender =>
                        Menus.HUDEnable && Menus.EnableDamageAPI && 
                        API._heroesinfo.Where(x => x.Hero == Hero).FirstOrDefault() != null &&
                        API._heroesinfo.Where(x => x.Hero == Hero).FirstOrDefault().Damage != 0,
                StartPositionUpdate = delegate
                {
                    return HPline.End;
                },
                EndPositionUpdate = delegate
                {
                    Vector2 v2 = HPline.End;
                    v2.X -= API._heroesinfo.Where(x => x.Hero == Hero).FirstOrDefault().Damage/Hero.MaxHealth * (0.82f - 0.07f) * scal;
                    if (v2.X <= HPline.Start.X) v2 = HPline.Start;
                    return v2;
                },
            };
            damageline.Add(6);
            Manaline = new Render.Line(new Vector2(xadd, 0), new Vector2(xadd, 0), (int)(8f * Menus.HUDScale), Menus.ManaBarColor)
            {
                ...
                StartPositionUpdate = delegate
                {
                    Vector2 v2 = new Vector2(Menus.HUDX + (7 * scal), Menus.HUDY + yadd + (73.5f * scal));
                    return v2;
                },
                EndPositionUpdate = delegate
                {
            ...
            var overlay00 = new Render.Sprite(Resources.ffx, new Vector2(xadd, 0))
            {
                ...
                {
                    Vector2 v2 = new Vector2(Menus.HUDX + (7 * scal), Menus.HUDY + yadd + (61f * scal));
                    return v2;
                },
            ...
            S1 = new Render.Sprite(S1bitmap, new Vector2(0, 0));
            S2 = new Render.Sprite(S2bitmap, new Vector2(0, 0));

            var black1 = new Render.Sprite(Resources.black60, new Vector2(0, 0));
            var black2 = new Render.Sprite(Resources.black60, new Vector2(0, 0));
            var black3 = new Render.Sprite(Resources.black60, new Vector2(0, 0));
            ...
            {
                Vector2 v2 = new Vector2(Menus.HUDX, Menus.HUDY + yadd);
                v2.X += scal * 60;
                v2.Y += scal * 6;
                return v2;
            };
            ...
            var isdead = new Render.Text((int)xadd, 0, "Dead", (int)(22 * Menus.HUDScale), Color.Red)
            {
                ...
                PositionUpdate = delegate
                {
                    Vector2 v2 = new Vector2(Menus.HUDX + 44.5f * Menus.HUDScale, Menus.HUDY + yadd + 69f * Menus.HUDScale);
                    return v2;
                },
                TextUpdate = delegate
                {
                    return loadondeath ? "?" : Menus.Format3(lastdeath + Menus.GetDeathTime(hero, Game.Time - 35f) - Game.Time);
                },
                OutLined = true,
                Centered = true
            };
            ...
            var Level = new Render.Text((int)xadd, 0, "", (int)(20 * Menus.HUDScale), Color.White)
            {
                ...
                PositionUpdate = delegate
                {
                    Vector2 v2 = new Vector2(Menus.HUDX + (44 * scal), Menus.HUDY + yadd + (46 * scal));
                    return v2;
                },
            ...
            var SummSpe_1 = new Render.Text((int)xadd, 0, "", (int)(24 * Menus.HUDScale), Color.DarkRed)
            {
                ...
                PositionUpdate = delegate
                {
                    Vector2 v2 = new Vector2(Menus.HUDX + (71 * scal), Menus.HUDY + yadd + (17 * scal));
                    return v2;
                },
            ...
            var SummSpe_2 = new Render.Text((int)xadd, 0, "", (int)(24 * Menus.HUDScale), Color.DarkRed)
            {
                ...
                PositionUpdate = delegate
                {
                    Vector2 v2 = new Vector2(Menus.HUDX + (71 * scal), Menus.HUDY + yadd + (44 * scal));
                    return v2;
                },
            ...
            //Drag = new Render.Sprite(Resources.cursor_drag, new Vector2(0, 0))
            //{
            //    PositionUpdate = delegate
            //    {
            //        Vector2 v2 = new Vector2(Menus.HUDX + (71 * scal), Menus.HUDY + yadd + (44 * scal));
            //        return v2;
            //    },
            //    VisibleCondition = sender => Menus.HUDEnable &&
            //};
        ...
        //private void DragFunction(WndEventArgs args)
        //{
        //    if (!Menus.HUDEnable)
        //        return;
        ...
        //    }
        //}
        private void DoubleClickFunction(WndEventArgs args)
        {
            ...
            if (isinsideR(Game.CursorPos2D) && isOff(SpellSlot.R) && Hero.Spellbook.GetSpell(SpellSlot.R).IsLearned)
            {
                ...
                //Chat.Print((int)Hero.PercentRespawnTimeMod + "");
        ...
        private void Game_OnUpdate(EventArgs args)
        {
            ...
            {
                //Chat.Print(Hero.ChampionName + " die");
                //Chat.Print(Hero.PercentRespawnTimeMod + "");
                //Chat.Print(Hero.DeathDuration + "");
                lastdeath = Game.Time;
                lastdeathcontrol = false;
            }
            if (!Hero.IsDead && !lastdeathcontrol)
            {
                //Chat.Print(Hero.ChampionName + " respawn");
                //Chat.Print(Hero.PercentRespawnTimeMod + "");
                //Chat.Print(Hero.DeathDuration + "");
                lastdeath = Game.Time;
                lastdeathcontrol = true;
                if (loadondeath) loadondeath = false;
            }
        }
